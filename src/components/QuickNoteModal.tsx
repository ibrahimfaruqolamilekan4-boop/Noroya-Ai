...
export default function QuickNoteModal({ 
  ...
  return (
    ...
    >
      <div 
        className="relative w-full max-w-lg bg-[#0f172a] border border-slate-800 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-150"
      ...
      >
        ...
        <div className="p-5 flex items-center justify-between border-b border-slate-850">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-[10px] text-slate-400 font-mono">APPEND TRADE CONTEXT INSTANTLY</p>
            </div>
          </div>
          <button 
            type="button"
            onClick={onClose}
            className="p-1.5 rounded-lg bg-slate-900 hover:bg-slate-850 text-slate-400 hover:text-slate-100 border border-slate-800 transition cursor-pointer"
          >
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <form onSubmit={handleSave} className="p-5 space-y-4">
          ...
          {/* Trade Selector */}
          <div>
            <label className="block text-[10px] uppercase font-mono text-slate-400 mb-1.5 font-bold tracking-wider">
              Select Position context
            </label>
            {sortedTrades.length === 0 ? (
              <div className="p-3 bg-slate-950 border border-slate-850 rounded-xl text-center text-xs text-slate-500 leading-normal">
              ...
              <div className="relative">
                <select
                  ...
                  className="w-full text-xs p-3 pr-8 bg-slate-950 border border-slate-850 rounded-xl text-slate-100 font-medium focus:outline-none focus:border-cyan-500 appearance-none cursor-pointer"
                >
                  {sortedTrades.map((t) => {
          ...
          {activeTrade && (
            <>
              ...
              <div className="p-2.5 bg-slate-950 border border-slate-900 rounded-lg flex items-center justify-between text-[11px] text-slate-400">
                <div className="flex gap-4">
                  <span>Entry: <code className="font-mono text-slate-200 font-semibold">{activeTrade.entry_price}</code></span>
              ...
              <div className="bg-slate-950 p-3 rounded-xl border border-slate-900 space-y-2">
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-grow p-2 text-xs bg-[#0f172a] border border-slate-850 rounded-lg text-slate-200 focus:outline-none focus:border-indigo-400"
                  />
                  <button
                    type="button"
                    onClick={handleAppendNotes}
                    className="px-3 bg-slate-900 hover:bg-slate-800 text-indigo-400 border border-slate-850 font-bold text-xs rounded-lg flex items-center gap-1 cursor-pointer transition"
                  >
                    <span>Append</span>
              ...
              <div>
                <label className="block text-[10px] uppercase font-mono text-slate-400 mb-1 font-bold tracking-wider">
                ...
                <textarea
                  ...
                  className="w-full min-h-[140px] p-3 text-xs bg-slate-950 border border-slate-850 rounded-xl text-slate-200 focus:outline-none focus:border-cyan-500 font-sans leading-relaxed"
                />
              </div>
            </>
          )}
          ...
          {activeTrade && (
            <div className="flex gap-3 pt-2 border-t border-slate-850">
              <button
                type="button"
                onClick={onClose}
                className="w-2/5 p-2.5 bg-slate-900 hover:bg-slate-850 border border-slate-800 font-bold rounded-xl text-xs text-slate-350 cursor-pointer text-center transition"
              >
                Clear / Exit
              </button>
              <button
                ...
                className="w-3/5 p-2.5 bg-indigo-650 hover:bg-indigo-600 border border-indigo-500 font-bold rounded-xl text-xs text-white cursor-pointer transition flex items-center justify-center gap-2 disabled:opacity-45"
              >
                {isSubmitting ? (
